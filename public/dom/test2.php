...
//
//         $img_path = dirname(__DIR__) . '/uploads/' . date('ymd') . '/';
//
//         if (!is_dir($img_path)) {
...
//     file_put_contents($filename, $result);
//
//     $xinarc = str_replace(dirname(__DIR__), "http://php-study.test", $filename);
//     $new_src[] = $xinarc;
...
if ($res->count()) {
    foreach ($res as $k => $img) {
        ...
        $host = 'https://mbd.baidu.com';
        $result = curl_get_content($src, $host);
        ...
        $filename = $img_path . md5($src);
        file_put_contents($filename, $result);
        $xinarc = str_replace(dirname(__DIR__), "http://php-study.test", $filename);

        pq($img)->attr('src', $xinarc);
    }
}
...
$sql = "insert into cid(yid, content) values('a', '".$content."')";
